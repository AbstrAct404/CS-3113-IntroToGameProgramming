...
void processInput() 
{
    ...
    bool isMouseInTeardrop = isColliding(&gMousePosition, &gScale, 
                                        &gPosition, &gScale);
    // TODO - If the player presses left mouse button within teardrop, release
    //        teardrop (i.e. use its movement vector)
    if (isMouseInTeardrop && IsMouseButtonPressed(MOUSE_LEFT_BUTTON))
...
void update() 
{
    ...
    if (gTeardropStatus == HANGING)
        {
            gPosition.x = gPosition.x + SPEED * cos(GetTime()) * deltaTime;
        } // moving back and forth on the X-AXIS
    ...
    {
        if(gScale.x > 0) gScale.x = gScale.x - SHRINK_RATE * deltaTime;
        if(gScale.y > 0) gScale.y = gScale.y - SHRINK_RATE * deltaTime;
    }
